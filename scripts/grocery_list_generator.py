...
CATEGORY_KEYWORDS: Dict[str, List[str]] = {
    "Produce": [
        "lettuce", "spinach", "arugula", "greens", "salad",
        "tomato", "cucumber", "onion", "garlic", "pepper", "bell pepper",
        "carrot", "celery", "broccoli", "cauliflower", "cabbage",
        "potato", "sweet potato", "avocado", "lime", "lemon",
        "apple", "banana", "berry", "berries", "fruit",
        "cilantro", "parsley", "basil", "herb", "kale",
        "zucchini", "squash", "mushroom", "corn", "pea",
    ],
    "Meat": [
        "chicken", "beef", "pork", "turkey", "sausage",
        "bacon", "ham", "lamb", "steak", "ground beef",
        "fish", "salmon", "tuna", "tilapia", "cod", "mahi-mahi",
        "shrimp", "seafood",
    ],
    "Dairy": [
        "milk", "cream", "cheese", "cheddar", "mozzarella",
        "parmesan", "feta", "yogurt", "butter", "egg",
        "sour cream", "cottage cheese",
    ],
    "Pantry": [
        "oil", "olive oil", "vegetable oil", "vinegar",
        "flour", "sugar", "salt", "pepper", "spice", "seasoning",
        "rice", "pasta", "quinoa", "oats", "cereal",
        "can", "canned", "beans", "black beans", "chickpeas",
        "stock", "broth", "sauce", "salsa", "hot sauce",
        "honey", "maple syrup", "peanut butter",
        "cumin", "paprika", "chili powder", "garlic powder",
        "mustard", "ketchup", "mayo", "mayonnaise",
        "soy sauce", "balsamic", "dijon",
    ],
    "Frozen": [
...
def clean_ingredient_name(name: str) -> str:
    ...
    name = re.sub(r',\s*(diced|chopped|sliced|minced|halved|thinly sliced|optional|for serving|drained and rinsed).*$', '', name)
    ...
    name = re.sub(r'^(fresh|dried|frozen|canned|shredded|chopped|sliced|diced|minced)\s+', '', name, flags=re.IGNORECASE)
...
def load_recipe_ingredients(recipe_path: Path) -> List[Tuple[str, str, str]]:
    ...
    with open(recipe_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if in_ingredients_section and line.startswith('## ') and 'Ingredients' not in line:
                break
            ...
            if in_ingredients_section and (line.startswith('-') or line.startswith('*')):
...
def generate_grocery_list_from_recipes(
    ...
    for recipe_name in recipe_names:
        ...
        recipe_filename = recipe_name if recipe_name.endswith('.md') else f"{recipe_name}.md"
...
def generate_grocery_list_from_meal_plan(meal_plan_path: Path) -> str:
    ...
    with open(meal_plan_path, 'r', encoding='utf-8') as f:
        ...
        for match in matches:
            ...
            if match.strip() and not any(skip in match.lower() for skip in [
                'no ', 'recipe available', 'breakfast', 'lunch', 'dinner',
